...
const GENRES = ["桌游", "冒险", "益智", "知识问答", "教育"];
const THEMES = ["海盗", "动物", "机器人", "历史", "忍者"];
const DIRECTIONS = [
...
function normalizeStaff(member: Staff): Staff {
  ...
  const statTotal = member.code + (member.scenario ?? 0) + member.art + member.sound;
  return {
    ...member,
...
export default function Home() {
  ...
  const [research, setResearch] = useState(STARTING_RESEARCH);
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    const migrateUntouchedOpening = (saved.balanceVersion ?? 1) < BALANCE_VERSION &&
      saved.year === 1 && saved.month === 4 && saved.week === 1 && saved.cash === 5000 &&
      !saved.project && (saved.releases?.length ?? 0) === 0 && (saved.staff?.length ?? 0) === 4;
    setCash(migrateUntouchedOpening ? STARTING_CASH : saved.cash);
    setFans(migrateUntouchedOpening ? STARTING_FANS : saved.fans);
    setResearch(migrateUntouchedOpening ? STARTING_RESEARCH : saved.research);
    setYear(saved.year);
    setMonth(saved.month);
    setWeek(saved.week);
    const migratedStaff = (migrateUntouchedOpening ? INITIAL_STAFF : saved.staff ?? INITIAL_STAFF).map(normalizeStaff);
    ...
    setReputation(migrateUntouchedOpening ? STARTING_REPUTATION : saved.reputation ?? STARTING_REPUTATION);
    setGenreExperience(saved.genreExperience ?? {});
    setThemeExperience(saved.themeExperience ?? {});
    setUnlockedGenres(migrateUntouchedOpening ? GENRES : saved.unlockedGenres ?? GENRES);
    setUnlockedThemes(migrateUntouchedOpening ? THEMES : saved.unlockedThemes ?? THEMES);
  ...
  const totalPower = useMemo(
    () => staff.reduce((sum, member) => sum + member.code + member.scenario + member.art + member.sound, 0),
    [staff],
  );
  ...
  const restartGame = () => {
    ...
    window.localStorage.removeItem("pixel-studio-save");
  ...
  const finishProject = (finished: Project) => {
    ...
    const scores = getReviewScores({
      ...
      reputation,
      bugs: finished.bugs,
      randomValues: [Math.random(), Math.random(), Math.random(), Math.random()],
    });
    ...
    const sales = getFirstWeekSales({
      ...
      contentPopularity: (finished.contentPopularity ?? getContentPopularity(finished.genre, finished.theme, isGreatCombo)) *
        getReleaseFatigueMultiplier(releases, finished.genre, finished.theme),
    ...
    setResearch((value) => value + getDirectionConfig(finished.direction).research);
    setReleases((items) => {
  ...
  useEffect(() => {
    ...
    if (year >= 20 && !endingShown) {
      const bestSeller = [...releases].sort((a, b) => b.sales - a.sales)[0];
      const bestProfit = [...releases].sort((a, b) =>
        (b.income - (b.developmentCost ?? 0)) - (a.income - (a.developmentCost ?? 0)),
      )[0];
      const score = Math.round(cash);
      ...
      setEventData({
        kind: "ending",
        title: "20 年经营报告",
        headline: score >= 20_000 ? "传奇工作室诞生！" : score >= 5_000 ? "跻身一流开发商！" : "故事仍会继续",
    ...
    if (month === 3 && week === 1) {
      const payroll = getAnnualPayroll(staff);
    ...
    if (month === 12 && week === 1) {
      ...
      const awardsUnlocked = releases.some((item) => item.score >= HALL_OF_FAME_SCORE);
      ...
      let prize = 0;
      let newFans = 0;
      const gotyChance = best && best.score >= 36 ? Math.min(.82, .18 + (best.score - 36) * .16) : 0;

      if (awardsUnlocked && best && best.score >= 36 && Math.random() < gotyChance) {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (isNewWeek) {
        ...
        if ((nextReleases[0]?.weeks ?? 0) > 16 && (nextReleases[0]?.weeks ?? 0) % 4 === 0) {
          setFans((value) => Math.max(0, Math.round(value * .98)));
        }
      ...
      setStaff((members) => members.map((member) => {
        if (!project || member.resting) {
          const nextEnergy = clamp(member.energy + (project ? 11 : 14), 0, 100);
          return { ...member, energy: nextEnergy, resting: project ? nextEnergy < 95 : false };
        }
        const nextEnergy = clamp(member.energy - 100 / (Math.max(8, member.maxPower) * 3.2), 0, 100);
        return { ...member, energy: nextEnergy, resting: nextEnergy <= 10 };
      }));
      ...
      if (project) {
        ...
        const activeTotalPower = staff.reduce((sum, member) => member.resting
          ? sum
        ...
        setProject((current) => {
          ...
          if (current.kind === "game") {
            ...
            if (stage === "debug") {
              const stagePower = getStageTeamPower(staff, "debug");
              const debugGain = getDebugGain(stagePower, energyModifier, Math.random());
              const bugs = Math.max(0, current.bugs - debugGain);
              const researchProgress = (current.debugResearch ?? 0) + (current.bugs - bugs) * 1.5;
              const gainedResearch = Math.floor(researchProgress);
              if (gainedResearch > 0) setResearch((value) => value + gainedResearch);
              const next = {
                ...current,
                bugs,
            ...
            const leadIsResting = current.leadStaffId
              ? staff.find((member) => member.id === current.leadStaffId)?.resting
              : false;
            const leadSkill = leadIsResting ? 0 : current.leadSkill;
            const stagePower = getStageTeamPower(staff, stage);
            const gain = getDevelopmentGain(stagePower, leadSkill, directionModifier, energyModifier, Math.random());
            const qualityModifier = directionConfig.quality;
            const qualityGain = getQualityGain(stagePower, leadSkill, qualityModifier, Math.random());
            ...
            let next = {
              ...
              elapsedWeeks: (current.elapsedWeeks ?? 0) + (isNewWeek ? 1 : 0),
              fun: current.fun + (stage === "planning" ? qualityGain * .65 : stage === "coding" ? qualityGain * .35 : 0),
              creativity: current.creativity + (stage === "planning" ? qualityGain : stage === "graphics" ? qualityGain * .12 : 0),
              graphics: current.graphics + (stage === "graphics" ? qualityGain * 1.05 : 0),
              sound: current.sound + (stage === "sound" ? qualityGain * 1.1 : 0),
              bugs: current.bugs + (stage === "coding"
                ? (current.direction === "赶工" ? Math.random() * 1.05 : Math.random() * .65) * polishProtection
                : Math.random() * .12 * polishProtection),
            };
            if (
              isNewWeek &&
              (next.eventCount ?? 0) < 2 &&
              ...
              const event = [
                ...
                },
                {
                  ...
                  reward: "趣味 +13 · 创意 +13",
                  apply: () => ({ ...next, fun: next.fun + 13, creativity: next.creativity + 13 }),
                  fans: 0,
                  research: 0,
                },
                {
                  ...
                  apply: () => ({
                    ...next,
                    stageProgress: Math.max(0, (next.stageProgress ?? 0) - 25),
                    progress: Math.max(0, next.progress - 25),
                    bugs: next.bugs + 5,
                  }),
                  fans: 0,
                  research: 0,
            ...
            if ((next.stageProgress ?? 0) >= (next.stageTarget ?? 1)) {
              ...
              if (nextStage === "debug") {
                ...
                return {
                  ...
                  leadName: "全体员工",
                  leadSkill: getStageTeamPower(staff, "debug"),
                };
              }
              window.setTimeout(() => {
                setPaused(true);
          ...
          const gain = (activeTotalPower / 22) * directionModifier * energyModifier * (0.85 + Math.random() * 0.3);
  ...
  const levelUp = (id: number) => {
    ...
    const unlockedNow = unlockRules.filter((rule) => rule.role === member.role && rule.level === nextLevel);
  ...
  const advertise = (cost: number, hype: number, name: string, segment: keyof FanSegments) => {
    ...
    if ((!project || project.kind !== "game") && !latestRelease) return announce("先发售一款游戏再开展宣传");
    if (cash < cost) return announce("资金不足");
    const previousUses = project?.kind === "game"
      ? project.advertisingUses?.[name] ?? 0
      : latestRelease?.advertisingUses?.[name] ?? 0;
    const diminishing = Math.max(.15, 1 - previousUses * .25);
    const effectiveHype = Math.max(1, Math.round(hype * diminishing));
    setCash((value) => value - cost);
    if (project?.kind === "game") {
      setProject((current) => current ? {
        ...current,
        hype: current.hype + effectiveHype,
        advertisingUses: { ...(current.advertisingUses ?? {}), [name]: previousUses + 1 },
      } : current);
    } else if (latestRelease) {
      const addedDemand = Math.round(
        Math.max(latestRelease.weeklySales ?? 0, latestRelease.sales * .04) * (effectiveHype / 8),
      );
      setReleases((items) => items.map((item, index) => index === 0 ? {
        ...item,
        remainingDemand: (item.remainingDemand ?? 0) + addedDemand,
        advertisingUses: { ...(item.advertisingUses ?? {}), [name]: previousUses + 1 },
      } : item));
    }
    setFans((value) => value + Math.round(effectiveHype * 2.5));
    setFanSegments((segments) => ({ ...segments, [segment]: segments[segment] + Math.max(1, Math.round(effectiveHype / 2)) }));
    announce(previousUses > 0 ? `${name}再次投放，效果开始递减` : `${name}引发了话题！`);
  };
    ...
    <main className="game-page">
      <div className="game-shell">
        ...
        <div className="utility-row">
          ...
          <button onClick={restartGame}>新开公司</button>
